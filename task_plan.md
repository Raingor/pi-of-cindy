...
- [x] P4 记忆页补齐（见 git log）
  - § 分段解析 + `<!-- created=…, last=… -->` 日期标记提取（pws parseMemoryEntries 同款，
    取代自创的「- 前缀」行解析——旧解析与 pi 真实记忆文件格式不符）
  - 按日期分组（新日期在前、无日期垫底）+ 默认展开前 3 组 + 用户折叠优先
  - 条目全文渲染（去 200 字截断）+ 轻量行内 markdown（**粗**/`代码`/链接）
  - 搜索高亮（<mark> 黄底）随条目渲染
  - 复制/删除 hover actions（复制已有,补 copied 状态文案）
  - 实机验证：MEMORY.md 11 条目按 4 个日期组渲染、USER.md 13 条目、分组折叠/展开正常
- [x] P5 仪表盘补齐（见 git log）
  - 基线核对后发现拉取的远端提交已补齐大部分（StatCard ≈ 副标题、缓存命中率进度条、
    逐日图含 cost 线、CSV 导出、趋势对比、排序、分页、货币切换）
  - 本次补两处残余差距：自动刷新默认开启（pws 同款）+ 刷新间隔选择器 5/10/30/60s
  - codex-usage-status 维持登记平台例外（Cindy pi-only 无 Codex 账号数据源）
- [x] P6 子代理补齐（见 git log）
  - 基线核对：编辑 model/thinking、保存/取消/成功反馈已在（远端补齐）
