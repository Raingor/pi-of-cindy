// ensureBinary 兜底编排的集成测试：上游失败 → ripgrep 回退 CDN。
// 2026-08-31 只保留 pi harness:claude/codex 已无 pin 与下载链,用例随 pin 删除
// (pi 是目录分发,不走单文件 CDN fallback,无对应用例)。
//
...
const dirsToClean = [path.join('apps', 'ripgrep-bin', PLATFORM)];
...
before(async () => {
  server = http.createServer((req, res) => {
    ...
    } else if (u === `/ripgrep/${RIPGREP_PIN}/${PLATFORM}/rg.gz`) {
      res.end(GZ);
    } else {
      res.writeHead(404);
      res.end('nf');
